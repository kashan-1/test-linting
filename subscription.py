...
device_browser_url = "http://media.bizj.us/view/img/1217421/screen-shot-2013-10-16-at-124757-pm.png"
...
databricks_url = "https://upload.wikimedia.org/wikipedia/commons/6/63/Databricks_Logo.png"
...
chargebee_url = "https://upload.wikimedia.org/wikipedia/commons/0/01/Chargebee-logotype.png"
...
with Diagram("Digital Services Platform (Subscription) Network Architecture", show=False, graph_attr=graph_attr, edge_attr=edge_attr, node_attr=node_attr, direction="TB"):
...
#with Diagram("Digital Services Platform", show=False, direction="TB") as diag:
    ...
    with Cluster("Client Device", graph_attr=cluster_attr):
        ...
        sn_mobile >> Edge(xlabel="deep link") >> device_browser << Edge(xlabel="deep link") << hf_mobile
    ...
    with Cluster("VPC:siq-<env>-shared-infra", graph_attr=cluster_attr):
        ...
        transit_gateway >> Edge(color="black", ltail="cluster_VPC:siq-sso", lhead="cluster_VPC:siq-<env>-shared-infra") >> msk_vpc_endpoint1
    ...
    with Cluster("VPC:siq-dp", graph_attr=cluster_attr):
        ...
        databricks >> Edge(color="black", minlen="4", ltail="cluster_VPC:siq-dp", lhead="cluster_VPC:siq-<env>-shared-infra") >> msk_vpc_endpoint1
    ...
    with Cluster("VPC:siq-dsp", graph_attr=cluster_attr):
        ...
        with Cluster("Public Subnet"):
            ...
            subscription_mgmt_ext_waf >> Edge(xlabel="HTTP TLSv1.2+") >> subscription_mgmt_ext_alb
            subscription_mgmt_content_ext_waf >> Edge(xlabel="HTTP TLSv1.2+") >> subscription_mgmt_content_ext_alb
            subscription_mgmt_ext_alb >> subscription_mgmt_fe_api
            subscription_mgmt_content_ext_alb >> subscription_mgmt_content_fe_api
            subscription_mgmt_fe_api >> Edge(xlabel="HTTP TLSv1.3") >>  subscription_mgmt_be_alb
            subscription_mgmt_content_fe_api >> Edge(xlabel="HTTP TLSv1.3") >>  subscription_mgmt_content_be_alb
            ...
            chargebee_kafka_event_producer_ext_waf >> chargebee_kafka_event_producer_ext_alb >> \
                chargebee_kafka_event_fe_api >> Edge(xlabel="HTTP TLSv1.3") >> \
                chargebee_kafka_event_producer_be_alb >> \
                chargebee_kafka_event_producer >> Edge(xlabel="RW") >> aurora_db
            ec2_jumphost >> aurora_db
            wellness_score_kafka_consumer << Edge(xlabel="Consume") << msk_vpc_endpoint1
            chargebee_kafka_event_producer >> Edge(xlabel="Publish") >> msk_vpc_endpoint1
    ...
    with Cluster("VPC:siq-secrets", graph_attr=cluster_attr):
        ...
        aws_kms << secrets_manager << Edge(label = "Use Least\nPrevilage Access", color="black", minlen="1", ltail="cluster_VPC:siq-secrets", lhead="cluster_VPC:siq-dsp") << subscription_mgmt_content_fe_api
    ...
    with Cluster("Managed Services:Monitoring", graph_attr=cluster_attr):
        ...
        databricks >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:siq-dp", lhead="cluster_Managed Services:Monitoring") >> pagerduty
        secrets_manager >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:siq-secrets", lhead="cluster_Managed Services:Monitoring") >> pagerduty
        cognito_user_pool >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:siq-ecim", lhead="cluster_Managed Services:Monitoring") >> pagerduty
        msk_vpc_endpoint1 >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:siq-<env>-shared-infra", lhead="cluster_Managed Services:Monitoring") >> pagerduty
        aurora_db >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:siq-dsp", lhead="cluster_Managed Services:Monitoring") >> pagerduty
        webui_contents >> Edge(color="black", style="dotted", minlen="4", ltail="cluster_VPC:zepp", lhead="cluster_Managed Services:Monitoring") >> pagerduty

    with Cluster("VPC:Chargebee", graph_attr=cluster_attr):
        chargebee = Custom("", chargebee_icon)
        chargebee_offerings = Node("Product Catalog\nPlans\nPayment Gateway\nWebhooks\nAPI Keys\nCustomers\nSubscriptions\nFinancial Reports", shape="rectangle", height="3", width="2.8", fontsize="20"),
        chargebee_roles_and_perms = Node("Roles and Permissions", shape="rectangle", height="0.5", width="3.2", fontsize="20")
    ...
    with Cluster("Adyen Payment Gateway", graph_attr=cluster_attr):
        ...
        chargebee >> Edge(color="black", ltail="cluster_VPC:Chargebee", lhead="cluster_Ayden Payment Gateway") >> adyen_payment_gateway
    ...
    with Cluster("Azure AD", graph_attr=cluster_attr):
        azure_ad_identify_protection = ADIdentityProtection("Azure AD\nIdentity Protection")
        sn_hf_support_users >> Edge(xlabel="SSO + MFA\nEnabled", labelloc="c") >> azure_ad_identify_protection >> Edge(xlabel="Platform\nManagement") >> chargebee_roles_and_perms 
